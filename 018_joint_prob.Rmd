...
::: {.definition}
Le variabili aleatorie $X$ e $Y$ sono (incondizionatamente) indipendenti se la loro distribuzione congiunta è il prodotto delle rispettive distribuzioni marginali:
...
Solitamente, l'indipendenza condizionata viene indicata utilizzando la notazione $(A \indep B \mid C)$. Di seguito è riportato il modello grafico per una tale situazione.

![](images/AperpBmidC.png){width=50%}
...
La figura che abbiamo esaminato in precedenza, per rappresentare l'indipendenza condizionale di $A$ e $B$ dato $C$, è un esempio di rete bayesiana (BN, Bayesian network). Le reti bayesiane sono grafici diretti aciclici (Directed Acyclic Graph, DAG) usati per rappresentare un insieme di variabili casuali e le loro dipendenze condizionali. Ogni nodo disponibile nelle BN rappresenta una variabile. Definendo i genitori di un nodo come tutti quei nodi le cui frecce puntano al nodo stesso, si può introdurre la seguente suddivisione:

- i nodi radice sono tutti quei nodi che non hanno genitori;
- i nodi foglia sono tutti quei nodi che non hanno alcun figlio; non hanno cioè
nessuna influenza causale sulle altre variabili.

Ogni nodo ha associata una probabilità condizionata. Quando il nodo non è un nodo radice, cioè quando ha dei nodi genitori, la distribuzione di probabilita condizionale quantifica le dipendenze probabilistiche tra il nodo ed i suoi genitori. 
...
Le ipotesi di indipendenza condizionale rappresentate dal grafico indicano che la distribuzione congiunta completa di tutte le quantità $V$ ha una semplice fattorizzazione in termini di distribuzione condizionale $p( v \mid \text{genitori}[ v ])$ di ciascun nodo dati i suoi genitori, in modo che

$$
p(V) = \prod_{i=1}^n p(v \mid \text{genitori}[ v ]), \quad \forall v \in V.
$$

Ad esempio, quando inferiamo i parametri dai dati $y_1, \dots, y_n$, spesso assumiamo che le osservazioni siano i.i.d.. Utilizzando un BN possiamo rappresentare questa ipotesi come indicato nella figura seguente. 

![](images/plates_example_1.png){width=30%}

La figura illustra l'ipotesi che ogni osservazione $y_i$ sia stata generata indipendentemente dalla stessa distribuzione. Si noti che i dati sono condizionalmente indipendenti dal parametro $\theta$; marginalmente, invece, le osservazioni sono dipendenti. 
...
$$
p(\theta, \mathcal{D}) = p(\theta) \left[ \prod_{i=1}^n p(y_i \mid \theta) \right].
$$
