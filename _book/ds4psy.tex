...
Definiamo la funzione \texttt{likelihood()}
...
la quale, considerati come fissi i dati di \citet{zetschefuture2019}, ritorna l'ordinata della verosimiglianza binomiale per ciascun valore \texttt{param} in input.

La funzione \texttt{prior()}
...
ritorna l'ordinata della distribuzione a priori \(\mbox{Beta}(2, 10)\) per ciascun valore \texttt{param} in input.

La funzione \texttt{posterior()}
...
ritorna, per ciascun valore \texttt{param} in input, il prodotto della densità a priori e della verosimiglianza.
...
Possiamo dunque generare una catena di valori \(\theta\):
...
Mediante le istruzioni precedenti otteniamo una catena di Markov costituita da 10,001 valori. Escludiamo i primi 5,000 valori considerati come burn-in. I restanti 5,001 valori verranno considerati come un campione casuale estratto dalla distribuzione a posteriori \(p(\theta \mid y)\).
